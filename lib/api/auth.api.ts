...
export const authApi = {
    async login(data: LoginRequest): Promise<ApiResponse<LoginResponse>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to login. Please try again.',
            };
        }
    },
    ...
    async register(data: RegisterRequest): Promise<ApiResponse<User>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to register. Please try again.',
            };
        }
    },
    ...
    async logout(): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to logout. Please try again.',
            };
        }
    },
    ...
    async refreshToken(refreshToken: string): Promise<ApiResponse<{ token: string }>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Session expired. Please login again.',
            };
        }
    },
    ...
    async verifyEmail(token: string): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to verify email. Please try again.',
            };
        }
    },
    ...
    async resendVerification(email: string): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to resend verification. Please try again.',
            };
        }
    },
    ...
    async forgotPassword(email: string): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to process request. Please try again.',
            };
        }
    },
    ...
    async resetPassword(token: string, password: string): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to reset password. Please try again.',
            };
        }
    },
    ...
    async getMe(): Promise<ApiResponse<User>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to fetch user information.',
            };
        }
    },

    async loginWithGoogle(data: GoogleLoginRequest): Promise<ApiResponse<LoginResponse>> {
        try {
            logger.info('Logging in with Google', { email: data.email });
            const response = await apiClient.post<ApiResponse<LoginResponse>>('/auth/google', data);
            return response.data;
        } catch (error: any) {
            // Error already sanitized by interceptor - safe to use
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to login with Google. Please try again.',
            };
        }
    },
