...
export const documentsApi = {
    ...
    async uploadDocument(data: UploadDocumentRequest): Promise<ApiResponse<Document>> {
        try {
            logger.info('Uploading document', { caseId: data.caseId, type: data.documentType });
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to upload document. Please try again.',
            };
        }
    },
    ...
    async deleteDocument(id: string): Promise<ApiResponse<void>> {
        try {
            ...
            return {
                success: false,
                error: error.response?.data?.error || 'Unable to delete document. Please try again.',
            };
        }
    },
