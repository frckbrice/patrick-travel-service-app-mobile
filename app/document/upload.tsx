...
import { View, StyleSheet, ScrollView, Alert, Text, Image, TouchableOpacity, Platform } from 'react-native';
...
export default function UploadDocumentScreen() {
    ...
    const pickDocument = useCallback(async () => {
        try {
            ...
            if (!result.canceled && result.assets[0]) {
                ...
                logger.info('Document selected', { fileName: file.name, size: file.size });
            }
        } catch (error) {
    ...
    const pickImage = useCallback(async () => {
        ...
        if (!permission.granted) {
            Alert.alert(t('documents.permissionRequired'), t('documents.photoPermissionNeeded'));
            return;
        }
    ...
    const takePhoto = useCallback(async () => {
        ...
        if (!permission.granted) {
            Alert.alert(t('documents.permissionRequired'), t('documents.cameraPermissionNeeded'));
            return;
        }
    ...
    const handleUpload = useCallback(async () => {
        ...
        try {
            ...
            if (response.success) {
                logger.info('Document uploaded successfully', { documentId: response.data?.id });
    ...
    }, [selectedCaseId, fileUri, fileName, documentType, fileSize, mimeType, t, router]);
            ...
            <View style={styles.content}>
                ...
                <Animated.View entering={FadeInDown.delay(100).duration(400)}>
                    ...
                    <SegmentedButtons
                        ...
                        buttons={[
                            { value: DocumentType.PASSPORT, label: 'Passport', icon: 'passport' },
                            { value: DocumentType.ID_CARD, label: 'ID', icon: 'card-account-details' },
                    ...
                    <View style={styles.uploadOptions}>
                        ...
                        <TouchableOpacity style={styles.uploadOption} onPress={pickImage}>
                            <View style={[styles.uploadIconContainer, { backgroundColor: COLORS.success + '15' }]}>
                                <MaterialCommunityIcons name="image" size={32} color={COLORS.success} />
                            </View>
                            <Text style={styles.uploadOptionText}>Gallery</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.uploadOption} onPress={pickDocument}>
                            <View style={[styles.uploadIconContainer, { backgroundColor: COLORS.warning + '15' }]}>
                                <MaterialCommunityIcons name="file-document" size={32} color={COLORS.warning} />
                            </View>
                                ...
                                {isImageFile && fileUri ? (
                                    ...
                                    <View style={styles.filePreview}>
                                        <MaterialCommunityIcons
                                            name={mimeType.includes('pdf') ? 'file-pdf-box' : 'file-document'}
                                            size={64}
                ...
                <Animated.View entering={FadeInDown.delay(300).duration(400)}>
                    ...
                    <Button
                        title={isUploading ? `${t('documents.uploading')} ${uploadProgress}%` : t('documents.uploadDocument')}
